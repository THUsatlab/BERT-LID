...
class BertCNN(BertPreTrainedModel):
    ...
    def __init__(self, config, num_labels, n_filters, filter_sizes):
        ...
        self.convs = Conv1d(80, n_filters, filter_sizes)

        self.classifier = nn.Linear(len(filter_sizes) * n_filters, num_labels)
        self.apply(self.init_bert_weights)
        print('num_labels,Dropout,n_filters,filter_sizes', num_labels,
              config.hidden_dropout_prob, n_filters, filter_sizes)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        print('pre, true:', logits.view, labels.view(-1), cat)
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            print('pre, true:', logits.view(-1, self.num_labels), labels.view(-1))
            return loss
        else:
            return logits
